...
df_index_gdp_weighted <- df_index_real %>%
  ...
  filter(!country_iso3 %in% c("VEN", "SLE", "LBR", "ZMB", "COD", "MDG", "ISL")) %>%
  group_by(country_iso3, year) %>%
  reframe(
...
```{r}
write_csv(df_index_gdp_weighted, "../../data/processed/_03_Index.csv")
```

## Quick Plot

```{r}
p <- df_index_gdp_weighted %>%
  filter(
    country_iso3 %in% c(
