...
Uncomment the third line to use total trade data instead of net exports (but then also adjust the weighting and index calculation accordingly)
```{r}
df_prices <- read_csv("../../data/intermediate/01_Commodity_Prices.csv")
df_trade <- read_csv("../../data/intermediate/02_Trade.csv")
# df_trade <- read_csv("../../data/intermediate/02_Trade_total.csv")
```
## Filter trade data

Filtering country + comm series that are 
- incomplete over the whole period
- less than 10% of overall exports

following Collier & Goderis (explained in Appendix I)

```{r}
df_trade_filter <- df_trade %>%
  # filter(country_iso3 == "BRA") %>%
  group_by(country_iso3, commodity) %>%
  reframe(
    n_years = n(),
    total_value = sum(export_value_usd, na.rm = TRUE)
  ) %>%
  filter(
    n_years == 29 | total_value / sum(total_value) > 0.1
  ) %>%
  reframe(
    country_iso3,
    commodity,
...
```{r}
df_trade <- df_trade %>%
  left_join(df_trade_filter, by = c("country_iso3", "commodity")) %>%
  filter(keep == TRUE) %>%
  select(-keep)
```
...
df <- df_trade %>%
  ...
  select(
    ...
    year,
    commodity,
    ## adjust the coments wheter its net or total trade data! 
...
```{r}
df_wdi <- read_csv("../../data/raw/WDI.csv")
```
...
Unrealistic: countries with very high gdp weights (>1), ie: QAT,...
...
```{r}
write_csv(df_cleaned, "../../data/intermediate/03_Index.csv")
haven::write_dta(df_cleaned, "../../data/intermediate/03_Index.dta")
```

## Development of the index after each step
