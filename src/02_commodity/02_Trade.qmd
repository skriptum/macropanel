...
```{r}
write_csv(baci_hs4_net, "../../data/intermediate/02_Trade.csv")
```
...
baci_hs4 <- map_dfr(years, function(yr) {
  ...
  # Process: add country names, convert HS6 → HS4, aggregate
  df <- raw_data %>%
    left_join(country_names, by = c("i" = "country_code")) %>%
    mutate(HS4 = str_sub(k, 1, 4)) %>%
    group_by(country_iso3, HS4) %>%
...
```{r}
write_csv(baci_hs4, "../../data/intermediate/02_Trade_total.csv")
```
