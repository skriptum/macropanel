...
```{r}
raw_PUB <- read_csv("../../data/raw/scientific-publications-per-million.csv")

df_PUB <- raw_PUB %>%
  select(Code, Year, `Scientific and technical journal articles per million people`) %>%
  reframe(
    ISO3 = Code,
    year = as.integer(Year),
    pub_per_million = as.numeric(`Scientific and technical journal articles per million people`)
  )
```
...
df_comb <- df_oec %>%
  ...
  full_join(df_PUB, by=c("ISO3", "year"))
```
...
desc <- c(
  ...
  pub_per_million = "Scientific and technical publications per million people"
)
```
